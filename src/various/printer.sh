...
_main_printer-sourceDir() {
    ...
    printStarted-scriptName$(zsf)() {
      print-headline-message$(zsf) "STARTED: " "$1"
    }

    printFinished-scriptName$(zsf)() {
      print-headline-message$(zsf) "FINISHED: " "$1"
    }
    
    print-successMessage$(zsf)() {
      isLastCommandSucceed$(zsf) && print-headline-message$(zsf) "SUCCESS" "$1" || print-errorMessage$(zsf) "$1"
    }
    
    print-errorMessage$(zsf)() {
      print-headline-message$(zsf) "ERROR in <$funcstack[2] <- $funcstack[3] <- $funcstack[4]>" "$1"
    }

    print-exceptionMessage$(zsf)() {
      print-headline-message$(zsf) "EXCEPTION: " "$1"
    }

    print-warningMessage$(zsf)() {
      print-headline-message$(zsf) "WARNING" "$1"
    }

    print-headline-message$(zsf)() {
